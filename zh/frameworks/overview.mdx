---
title: "框架集成总览"
description: "先区分 Server inference、Pydantic AI adapter、LangGraph node 和 LangChain middleware，再选择接入路径。"
---
...
PowerContext 有四条 Python 框架路径。它们共用 Server contract，但不是同一种抽象。尤其要分清：Server 内的 Pydantic AI inference，与面向 Pydantic AI Agent 的 Preview adapter 是两条独立路径。
...
| 需求 | 下一页 |
|---|---|
| 配置 Source extraction、Experience / Skill / Handoff generation、embedding 或 reranking | [Pydantic AI Server inference](/zh/frameworks/pydantic-ai-server-inference) |
| 查看 Preview Capability、Toolset 与 capture lifecycle | [Pydantic AI Agent adapter](/zh/frameworks/pydantic-ai) |
| 为 async graph 增加三个 Memory tools 和临时 recall | [LangGraph](/zh/frameworks/langgraph) |
| 增加 request-level recall，并可选 capture completed-turn Source | [LangChain](/zh/frameworks/langchain) |
| 不使用框架 lifecycle hook，只暴露部分显式操作 | MCP；没有 automatic prepare、capture 或 flush |
...
下一页先看 [Pydantic AI Server inference](/zh/frameworks/pydantic-ai-server-inference)，避免把 Server 模型配置与 Preview Agent adapter 混在一起。
