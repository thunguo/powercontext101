...
模型可以提交 <Tooltip headline="Candidate" tip="Experience 或 managed Skill 的提案。pending 状态进入 Review Inbox，批准前不是 Artifact。" cta="阅读 Experience 与 Skill" href="/zh/mental-model/experience-and-skill">Candidate</Tooltip>。Candidate 是待审核的填，模型只能交提案，不能自己生效。Candidate 被 <Tooltip headline="Review" tip="把提案写成不可变 Artifact Revision，或记录拒绝原因。两种决定都是终态。" cta="阅读 Experience 与 Skill" href="/zh/mental-model/experience-and-skill">Review</Tooltip> 通过后才形成不可变的 Artifact Revision（审核通过后留下的不可变版本）。<Tooltip headline="Skill" tip="获批不会安装或执行。必须显式导出一个精确的 approved Revision。" cta="阅读 Experience 与 Skill" href="/zh/mental-model/experience-and-skill">Skill</Tooltip> 获批后仍要显式导出，它不会自行安装或执行。

<Tooltip headline="PreparedContext" tip="一次 Agent turn 的有界临时视图。它不是新的长期记录。召回失败必须 fail-open。" cta="阅读 fail-open" href="/zh/mental-model/fail-open">PreparedContext</Tooltip> 是本轮临时拼给模型看的有界上下文，不是长期的记录。
