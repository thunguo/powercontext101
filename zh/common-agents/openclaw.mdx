...
1. 在固定 Scope 里用 `powercontext_memory_store` 保存答案。五个 Memory tool 里只有 store 负责这次写入。
2. 开一个新的 private / direct session，再问同一句。
3. 召回必须带 Citation，并且只作为这次 latest-user prepare 的不可信历史进入模型输入。
4. 停掉 Server 后再问。自动 recall 可以失败，会话应继续。
