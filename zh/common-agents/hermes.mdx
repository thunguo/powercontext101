...
上面的 `hermes powercontext remember preference` 是 CLI smoke。统一题用来确认 Citation 通道。Hermes 自动 recall 走 provider prefetch，不是 Codex 那种 `additionalContext`。
...
1. 在固定 Scope 里用 `powercontext_remember` 或 `/pc remember` 显式保存答案。CLI smoke `hermes powercontext remember` 不能代替这一步。
2. 开一个新交互会话，再问同一句。
3. 召回必须带 Citation，并且只出现在 Hermes provider prefetch 的上下文里，不是 Codex 那种 `additionalContext`。
4. 停掉 Server 后再问一次。prefetch 可以失败，普通对话应继续。
