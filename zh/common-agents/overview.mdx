---
title: "通用 Agent 总览"
description: "在 Hermes 与 OpenClaw 之间选路，并把安装、激活、Server 启动和诊断分开处理。"
---
...
本章讲两个有 first-class setup 命令的通用宿主：Hermes 和 OpenClaw。它们连接同一个 Server，但宿主合同不同。先选宿主，不要把一边的配置照搬到另一边。
...
WorkBuddy 有单独的 `setup` 和 doctor 命令，但不在 `setup select` 与 `doctor integrations` 使用的 first-class bulk catalog 里。存在 setup 命令，不代表它属于这个 catalog。101 的验证合同见 [WorkBuddy](/zh/coding-agents/workbuddy)。便携 Skill + MCP 包见 [Agent Plugin](/zh/coding-agents/agent-plugin)。
...
要用完整 provider surface，继续看 [Hermes](/zh/common-agents/hermes)；只需要较窄的 Memory 路径，就看 [OpenClaw](/zh/common-agents/openclaw)。
