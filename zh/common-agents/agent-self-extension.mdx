...
<Warning>
Hermes 和 OpenClaw **不能** managed export。你可以在那里整理 evidence 或做 Review，但导出必须换到 [Codex](/zh/coding-agents/codex) 或 [Claude Code](/zh/coding-agents/claude-code)。OpenClaw 没有 Review / Skill 工具面。
</Warning>
...
回到 [通用 Agent 总览](/zh/common-agents/overview)，选择负责提交和 Review 这些操作的宿主 surface。要真正 export，换到编程宿主：

<Columns cols={2}>
  <Card title="导出到 Codex" icon="terminal" href="/zh/coding-agents/codex" arrow="true" cta="打开 Codex">
    managed export target `codex` 是 **Supported**。Review 完成后在这里执行 `powercontext skill export`。
  </Card>
  <Card title="导出到 Claude Code" icon="terminal" href="/zh/coding-agents/claude-code" arrow="true" cta="打开 Claude Code">
    managed export target `claude_code` 是 **Supported**。不要和 setup 命令里的 `claude-code` 混用。
  </Card>
</Columns>
