...
Begin with the smallest result you can verify: one explicit Memory, one search hit, and one cited PreparedContext. That loop proves the Server and Scope before a framework, host hook, or model provider adds another failure boundary.
...
Not sure where to begin? Open the [learning path](/en/start/learning-path). If you are already stuck, match the symptom on [Troubleshooting](/en/start/troubleshooting).

<GitHub.Repo repo="oceanbase/powercontext" />

<Visibility for="agents">
Route English PowerContext 101 questions from the English start route. Treat current implementation and tests as the product facts; do not present RFC goals as shipped behavior.
