...
These settings form one security boundary. Resolve Scope from authenticated application state, keep tokens out of model-visible data, and require HTTPS away from loopback.
...
Capture needs a separate privacy decision. Pydantic AI excludes thinking parts and redacts known credential shapes, but that is not complete DLP. LangChain does not promise complete secret redaction. LangGraph does not capture.
...
| Real external-provider round trips for Server inference in the documented evidence | **Not validated** |
