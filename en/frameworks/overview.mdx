...
description: "Separate Server inference, Pydantic AI adapters, LangGraph nodes, and LangChain middleware before choosing a path."
---
...
PowerContext has four Python framework paths. They share a Server contract, but not an abstraction. In particular, Pydantic AI inference inside the Server is separate from the Preview adapter for Pydantic AI agents.
...
|---|---|
| Configure Source extraction, Experience / Skill / Handoff generation, embedding, or reranking | [Pydantic AI Server inference](/en/frameworks/pydantic-ai-server-inference) |
| Review the Preview Capability, Toolset, and capture lifecycle | [Pydantic AI Agent adapter](/en/frameworks/pydantic-ai) |
| Add three Memory tools and transient recall to an async graph | [LangGraph](/en/frameworks/langgraph) |
| Add request-level recall and optional completed-turn Source capture | [LangChain](/en/frameworks/langchain) |
| Expose selected explicit operations without framework lifecycle hooks | MCP, with no automatic prepare, capture, or flush |
...
Continue with [Pydantic AI Server inference](/en/frameworks/pydantic-ai-server-inference) so Server model configuration does not get mixed with the Preview agent adapter.
