...
<Warning>
Hermes and OpenClaw **cannot** managed-export a Skill. You may gather evidence or run Review there, but export only on [Codex](/en/coding-agents/codex) or [Claude Code](/en/coding-agents/claude-code). OpenClaw has no Review / Skill tool surface.
</Warning>
...
The useful outcome is not “the agent wrote a Skill.” It is a traceable chain from exact evidence to an inspected Candidate, an approved immutable Revision, and a host-local projection whose destination and permissions were chosen explicitly.
...
Return to [Common agents overview](/en/common-agents/overview) to choose the host surface that will submit and review these operations. To export, switch to a coding host:

<Columns cols={2}>
  <Card title="Export to Codex" icon="terminal" href="/en/coding-agents/codex" arrow="true" cta="Open Codex">
    The managed export target `codex` is **Supported**. Run `powercontext skill export` here after Review.
  </Card>
  <Card title="Export to Claude Code" icon="terminal" href="/en/coding-agents/claude-code" arrow="true" cta="Open Claude Code">
    The managed export target `claude_code` is **Supported**. Do not mix it with the `claude-code` setup name.
  </Card>
</Columns>
