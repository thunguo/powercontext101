...
Hermes has the widest host integration among the documented common-agent hosts. The MemoryProvider handles lifecycle hooks and tools. A separate command companion registers `/pc` and `/powercontext` early enough for interactive use.
...
- A real Hermes host-process E2E is **Not validated** here.

## Finish the shared check on this host

> What should happen after the OpenAPI contract changes?

1. Store the answer explicitly in a stable Scope with `powercontext_remember` or `/pc remember`. The CLI smoke `hermes powercontext remember` does not replace this step.
2. Open a new interactive session and ask the same question.
3. The recall must include a Citation and appear only in Hermes provider prefetch context, not in a Codex-style `additionalContext` payload.
4. Stop the Server and ask again. Prefetch may fail. Ordinary conversation should continue.
5. Call `powercontext_remember` again. The failure must stay visible. Do not pretend the write succeeded.
