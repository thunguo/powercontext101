...
Run once with defaults before changing them. This gives you a known local baseline for separating host configuration from Server or authentication failures.
...
OpenClaw exposes this exact Memory subset. If the task needs Handoff, Review, Experience, or Skill, choose another integration instead of trying to infer hidden tools.
...
Also run the learning-path question used by the other hosts. First store it with `powercontext_memory_store`:
...
Lifecycle recall, capture, and Flush fail open. Explicit tool failures return structured unavailable, rejected, or conflict results. Unlike Hermes, the OpenClaw HTTP client uses standard `fetch()` behavior; the documented behavior does not claim that redirects are refused.
...
- A real OpenClaw host-process E2E is **Not validated** here.

## Finish the shared check on this host

> What should happen after the OpenAPI contract changes?

1. Store the answer in a stable Scope with `powercontext_memory_store`. Of the five Memory tools, only store performs this write.
2. Open a new private / direct session and ask the same question.
3. The recall must include a Citation and enter the model only as untrusted history from this latest-user prepare.
4. Stop the Server and ask again. Automatic recall may fail. The session should continue.
5. Call `powercontext_memory_store` again. The failure must stay visible. Do not pretend the write succeeded.
