...
Use the comparison table to choose a path, not as a substitute for the host tutorial. Hermes fits work that needs a broader PowerContext surface. OpenClaw exposes only Memory and applies stricter private-session admission. WorkBuddy and Agent Plugin have independent paths; adding either to `setup select` does not create the same automatic behavior.
## Hermes or OpenClaw?

The deciding difference is scope, not the number of checkmarks. Hermes exposes more PowerContext operations; OpenClaw narrows the surface to Memory and applies its own session-admission rules.
...
Installation, activation, and runtime are separate states. A copied plugin can still be inactive; an active plugin can still point at a stopped Server; and a healthy Server says nothing about which Scope the host selected. Diagnose those states in that order, because each has a different repair.
## Execution flow
