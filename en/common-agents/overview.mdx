...
WorkBuddy has dedicated `setup` and doctor commands, but it is not in the first-class bulk catalog used by `setup select` and `doctor integrations`. Do not infer catalog membership from the existence of a setup command.
