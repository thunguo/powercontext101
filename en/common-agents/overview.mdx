...
description: "Choose between Hermes and OpenClaw, then keep installation, activation, Server startup, and diagnostics separate."
---
...
This section covers two general-purpose hosts with first-class setup commands: Hermes and OpenClaw. They connect to the same Server, but their host contracts are different. Pick the host first; do not copy settings from one into the other.
...
WorkBuddy has dedicated `setup` and doctor commands, but it is not in the first-class bulk catalog used by `setup select` and `doctor integrations`. Do not infer catalog membership from the existence of a setup command. The 101 verification contract is on [WorkBuddy](/en/coding-agents/workbuddy). The portable Skill + MCP package is on [Agent Plugin](/en/coding-agents/agent-plugin).
...
Continue with [Hermes](/en/common-agents/hermes) for the full provider surface, or [OpenClaw](/en/common-agents/openclaw) for the narrower Memory-only path.
