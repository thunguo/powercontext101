...
description: "A well-written model draft is not shipped. Review comes first. Export and execution stay later."
icon: award
keywords:
  - Experience
  - Skill
...
Completed work often yields two reusable things. One is judgment for a similar situation. The other is a repeatable procedure an agent can follow.

PowerContext stores the first as <Tooltip headline="Experience" tip="Approved judgment for similar situations. It enters Review as a Candidate." cta="Read Experience and Skill" href="/en/mental-model/experience-and-skill">Experience</Tooltip> and the second as managed <Tooltip headline="Skill" tip="An approved repeatable method. Export is still required. PowerContext grants no execution authority." cta="Read Experience and Skill" href="/en/mental-model/experience-and-skill">Skill</Tooltip>. Neither may move directly from model output to an approved Artifact.

The distinction changes the review question. For Experience, ask whether the lesson follows from the cited outcome and will help in a similar situation. For Skill, inspect whether another agent can execute the procedure safely, including its checks and permission boundary.
...
This is the same OpenAPI example.
...
A managed Skill contains a name, a description used by the host for discovery, instructions, and validation checks.
...
Both objects start as a proposal and wait for Review. A <Tooltip headline="Candidate" tip="An Experience or Skill proposal awaiting Review." cta="Read Experience and Skill" href="/en/mental-model/experience-and-skill">Candidate</Tooltip> enters <Tooltip headline="Review" tip="Approve creates an Artifact Revision. Reject records the reason and creates no result Artifact." cta="Read Experience and Skill" href="/en/mental-model/experience-and-skill">Review</Tooltip> first. Revise a proposal with `version`. Approval creates an Artifact Revision.

<img src="/assets/mental-review-gate-en.svg" alt="Candidates pass Review first. Revise with version. Approval creates an Artifact Revision." />

A Candidate has its own lifecycle. `pending` can be inspected, revised, approved, or rejected. `approved` is terminal and must include a `result_artifact`. `rejected` is also terminal. It must include a reason and creates no result Artifact.
...
A proposal can come from you, or a model can draft it from bounded evidence.

### Typed propose

When the content is already known, submit a structured proposal. The common operations are `propose_experience` and `propose_skill`.

No generation model is required. The result is still a pending Candidate.

### Model-backed generate

A model can draft a Candidate from bounded evidence. The common operations are `generate_experience` and `generate_skill`.
...
If one reviewer reads v1 and another revises it to v2 first, the older approve call conflicts.
...
Approve runs one database transaction. It locks the pending Candidate, validates `expected_version` and lineage, then creates or revises the Artifact. For Experience it also updates the approved-head search projection. Finally it marks the Candidate approved and records the exact `result_artifact`.
...
An Experience can enter PreparedContext only when all of these are true. The Candidate is approved. It is the current head of its Experience artifact. It uses the same Scope as the request. Search and the PreparedContext builder treat it as relevant.
...
Approval creates an exact Skill Revision, for example `skill/SKILL_ID@1`. Nothing changes in the host filesystem yet.
...
The current CLI export targets are `codex` and `claude_code`.
...
Common projection states include `unpublished`, `current`, `update_available`, `conflict`, `drifted`, and `incompatible`.
...
| external Skill | A host filesystem or registry package | Scan, resolve, then import or fork into the Candidate lifecycle |
...
Propose or generate this judgment.
...
Read Candidate v1 and revise the lesson to produce v2. An intentional `approve(expected_version=1)` should conflict. Read v2 again, then approve it to get `experience/EXPERIENCE_ID@1`.
...
Approval produces `skill/SKILL_ID@1`.

PreparedContext still does not contain it. That is the expected result.

### 5. Export

Export exact `@1` to each supported host directory. Inspect the `SKILL.md` content and confirm that `powercontext.json` refers to the same exact ArtifactRef.
...
1. Candidate version and Artifact Revision are different numbers.
2. Propose and generate stop at pending or no-op.
3. Approve and reject bind the inspected `expected_version`.
4. Experience recall uses only the approved current head.
5. Skill remains outside PreparedContext.
6. Approval, export, and execution stay three steps.
7. Codex and Claude Code are the only current export targets listed.
8. Local drift and conflict prevent overwrite.
...
<Columns cols={2}>
  ...
  <Card title="Finish one Review" icon="award" href="/en/basic-usage/experience-skill-review" arrow="true" cta="Start Experience / Skill Review">
    Submit a Candidate from exact evidence, check the version, then approve or reject.
  </Card>
</Columns>
